 ...
 * Communicates with service worker for downloads and webhook posting.
 */
...
(() => {
  ...
  const csvEscape = (value) => {
    ...
    const escaped = str.replace(new RegExp(DELIMITER.string, 'g'), DELIMITER.string + DELIMITER.string);
    return DELIMITER.string + escaped + DELIMITER.string;
  };
  ...
  /**
   * Force load all watch history by scrolling
   */
  const forceLoadWatchHistory = () => {
    return new Promise((resolve) => {
      console.log('[Prime Exporter] Scrolling to load all items...');
      ...
      const scrollInterval = setInterval(() => {
        ...
        if (scrollCount % 10 === 0) {
          console.log(`[Prime Exporter] Scrolling... (${scrollCount} scrolls, ${Math.round(elapsed / 1000)}s elapsed)`);
        }
  ...
  const toCSV = (items) => {
    const headers = Object.values(MSG.column);
    const headerRow = headers.map(csvEscape).join(DELIMITER.field);

    const dataRows = items.map((item) => {
      return [
        toDateOnly(item.dateWatched),
        item.type,
        item.title,
        item.episodeTitle,
        item.id,
        ...
        .map(csvEscape)
        .join(DELIMITER.field);
    });

    return [headerRow, ...dataRows].join(DELIMITER.record);
  };

  /**
   * Main export function - called by popup via message
   */
  const collectWatchHistory = async () => {
    console.log('[Prime Exporter] Starting export...');
    ...
    try {
      ...
      // Scroll to load all items (this triggers fetch calls that get intercepted)
      console.log('[Prime Exporter] Starting scroll to load more items...');
      ...
      console.log(`[Prime Exporter] Export complete: ${sortedItems.length} unique items`);
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === 'collectHistory') {
      console.log('[Prime Exporter] Received collectHistory request');
      
      // Wrap in try-catch for safety
      (async () => {
        try {
          const items = await collectWatchHistory();

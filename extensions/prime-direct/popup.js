/**
 * Prime Video Obsidian Importer - Popup Script
 * 
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function showResults(results) {
    ...
    if (results.scraped !== undefined) {
      html += `<p><strong>${results.scraped}</strong> items scraped from Prime Video</p>`;
    }
  ...
  async function checkCurrentTab() {
    try {
      ...
      // More flexible URL matching for Prime Video watch history
      const isPrimePage = 
        url.includes('primevideo.com') && url.includes('watch-history') ||
        url.includes('primevideo.com') && url.includes('watchhistory') ||
        url.includes('amazon.com') && url.includes('watch-history') ||
        url.includes('amazon.com') && url.includes('video') && url.includes('history');
      
      console.log('[Popup] Is Prime page:', isPrimePage);
      ...
      if (scanBtn) {
        scanBtn.disabled = false;  // Enable scan button when on Prime page
        console.log('[Popup] On Prime page - scan button enabled');
      }
      return true;
    } catch (error) {
  ...
  async function handleScan() {
    ...
    try {
      showStatus('Scanning page for watch history...', 'loading');
      ...
      const filterMode = document.querySelector('input[name="date-filter"]:checked')?.value || 'all';
      let scrollStartDate = null; // null = load all
      ...
      // filterMode === 'all' means no limit, scroll all
      
      const statusMsg = scrollStartDate 
        ? `Scrolling to load items from ${scrollStartDate}...` 
        : 'Scrolling to load all items... This may take 1-2 minutes.';
      ...
      console.log(`[Popup] Scanned ${allItems.length} items from Prime Video`);
      
      // Apply date filter (reuse filterMode from above)
      ...
      if (apiUrl && apiKey) {
        ...
        const existingResult = await ObsidianAPI.getExistingPrimeEntries(apiUrl, apiKey);
        if (existingResult.ok && existingResult.entries) {
          ...
          scannedItems = newItems;  // Store only new items
        }
      }
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (scannedItems && scannedItems.length > 0) {
        ...
        showStatus('Collecting watch history from Prime Video...', 'loading');
        ...
        const statusMsg = scrollStartDate 
          ? `Scrolling to load items from ${scrollStartDate}...` 
          : 'Scrolling to load all history items... This may take 1-2 minutes.';
        showStatus(statusMsg, 'loading');
        ...
        console.log(`[Popup] Scraped ${allItems.length} items from Prime Video`);
  ...
  if (scanBtn) {
    ...
    console.log('[Popup] scanBtn NOT found - old HTML cached?');
  }
  ...
  dateFilterRadios.forEach(radio => {
    radio.addEventListener('change', () => {
      updateDateInputStates();
      saveSettings();
      // Clear scanned items when filter changes
      scannedItems = null;
      previewSection.classList.add('hidden');
    });
  });
  ...
  await loadSettings();
  const isOnPrimePage = await checkCurrentTab();
  console.log('[Popup] Is on Prime page:', isOnPrimePage);
  console.log('[Popup] Scan button disabled state:', scanBtn?.disabled);
  
  // Double-check scan button is enabled if on Prime page
  if (isOnPrimePage && scanBtn) {
    scanBtn.disabled = false;
    console.log('[Popup] Force-enabled scan button');
  }
  
  // Auto-test connection if we have credentials
  if (apiKeyInput.value.trim() && isOnPrimePage) {
    testConnection();
  }
